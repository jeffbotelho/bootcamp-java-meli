...
//    JoiaRepository joiaRepository;
    ...
    public ResponseEntity<Joia> salvaJoia(@RequestBody Joia joia, UriComponentsBuilder uriBuilder) {
        ...
        // sera enviado os dados para o banco de dados onde sera salvo e as informacoes ficarao guardadas na minha variavel j
        //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        ...
        // caso sucesso sera gerado um status code 201 e o id gerado sera exibido na minha url que foi capturado pela minha variavel j
        //<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        return ResponseEntity.created(uri).body(j);
    }

    // deleta o item pelo id identificado por RequestParam e retorna um status code 204
    ...
    // O id e passado na URI atraves do RequestParam e os dados a serem att enviados via RequestBody
